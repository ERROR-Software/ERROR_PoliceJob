  ...
  options = {
    {
      ...
      icon = 'fa-solid fa-phone', -- Ajout de l'icône téléphone
      onSelect = function()
        if (not canCall) then
          return ESX.ShowNotification(_U("can_call_cop", Config.accueil['timeforcallpolice']), "error")
        end
        canCall = false
        ESX.ShowNotification(_U("call_success"))
        TriggerServerEvent("error:AlertPolice")
        SetTimeout(Config.accueil['timeforcallpolice'] * 1000, function()
          canCall = true
        end)
      end,
    },
    {
      ...
      icon = 'fa-solid fa-file-signature', -- Ajout de l'icône plainte
      onSelect = function()
        ...
        if not input then
          return ESX.ShowNotification(_U("complaint_error"), "error")
        end
        TriggerServerEvent(
          "error:SendDiscordLog",
          Config.accueil['webhook'].bot_name,
          Config.accueil['webhook'].message_title,
          (Config.accueil['webhook'].message):format(
            input[1],
            ESX.PlayerData.firstName .. " " .. ESX.PlayerData.lastName,
            input[2]
          ),
          Config.accueil['webhook'].footer,
          Config.accueil['webhook'].color,
          Config.accueil['complaint_webhook']
        )
      end
    },
  }
